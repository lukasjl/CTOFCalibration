 ...
 * @author gavalian
 */
public class DataTransportVersion1 {
    ...
    public DataTransportVersion1(){
        table.readFile(tableString);    
        decoder.addTranslationTable(table);
    }
    
    public  List<CTOFPaddleVersion1>  getCTOFPaddlesGEMC(EvioDataEvent event){
        List<CTOFPaddleVersion1> list = new ArrayList<CTOFPaddleVersion1>();
        if(event.hasBank("CTOF::dgtz")==true){
             EvioDataBank bank = (EvioDataBank) event.getBank("CTOF::dgtz");
            int nrows = bank.rows();
            for(int row = 0; row < nrows; row++){
                CTOFPaddleVersion1 p = new CTOFPaddleVersion1();                
    ...
    public  List<CTOFPaddleVersion1>  getCTOFPaddlesRAW(EvioDataEvent event){
        ...
        for(int loop = 0; loop < 48; loop++){
            ...
            if(selected.size()==4){
                CTOFPaddleVersion1 paddle= new CTOFPaddleVersion1();
                ...
                paddle.TDCL = (selected.get(2).getTDC()/1000.);
                paddle.TDCR = (selected.get(3).getTDC()/1000.); 
                
                double product = paddle.ADCR*paddle.ADCL;
                paddle.MeanADC = Math.sqrt(product);
                if(paddle.ADCL!=0){
                    double ratio = paddle.ADCR/paddle.ADCL;
                    paddle.LRatio = Math.log(ratio);
                    //System.out.println("  DATA = " + paddle.LRatio + "  " + (paddle.ADCR/paddle.ADCL) + "  " + ratio);
